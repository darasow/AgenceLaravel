...
<body>
    ...
    @endphp
<nav class="border-gray-200 bg-blue-300 text-white mb-10">
    <div class="w-full mx-auto">
        <div class="mx-2 flex flex-wrap items-center justify-between">
            <a href="#" class="flex">
                ...
                <span class="self-center text-lg font-semibold whitespace-nowrap">Agence</span>
            </a>
            ...
            <div class="hidden md:flex justify-between items-end w-full md:w-auto md:order-1" id="mobile-menu-3">
                <ul class="flex-col md:flex-row flex md:space-x-8 mt-4 md:mt-0 md:text-sm md:font-medium">
                    <li>
                        <a href="{{route('admin.propriete.index')}}" @class(["md:bg-transparent text-lg  block pl-3 pr-4 py-2 md:p-0 rounded", 'text-blue-700' => str_contains($route, 'propriete.')])
                            aria-current="page">Gestion des Biens</a>
                    </li>
                    <li>
                        <a href="{{route('admin.option.index')}}"
                            @class(["md:bg-transparent text-lg  block pl-3 pr-4 py-2 md:p-0 rounded", 'text-blue-700' => str_contains($route, 'option.')])>Gestion des Options</a>
                    </li>
                    <li>
                    @auth
                          <form action="{{route('auth.logout')}}" method="post">
                            @csrf
                              @method('delete')
                                  <button
                            class="text-white text-lg hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 block pl-3 pr-4 py-2 md:hover:text-blue-700 md:p-0">Se déconnecter</button>
                    ...
                    <li>
                        <a href="{{route('auth.profile')}}"
                            class="text-white text-lg hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 block p-2 md:hover:text-blue-700 md:p-0">Profile</a>
                    </li>
                </ul>
            </div>
        </div>
